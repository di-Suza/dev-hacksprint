...
module.exports.sendForgotPasswordOtp = async (email) => {
  let user = await Users.findOne({ email });

  if (!user) {
    throw new AppError(
      "User didn't exist with this email, Register first!",
      404,
    );
  }

  // check if otp already exists in OTP
  let otpData = await OTP.findOne({ email });
  const now = new Date();
  let currentCount = 0;
...
module.exports.verifyForgotPasswordOtp = async ({ email, otp }) => {
  let user = await Users.findOne({ email });
  if (!user) {
  ...
  let { otpMatched, verifyAttempts } = await verifyOtp(email, otp);
  if (otpMatched) {
    // if otp matched then delete otp
    await OTP.deleteOne({ email });
    // token and sign for forgot password only valid for 5 min
